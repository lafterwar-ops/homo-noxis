---
name: john
description: Perfect or independently diagnose a specified Homo Noxius manuscript chapter through architecture, causal logic, scoring, iterative rewrite, adversarial audit, continuity repair, compression, and freeze. Use when the user invokes John(chapter N), John brainstorm(chapter N), asks to run or continue John, or requests the established chapter-perfection process. Keep completed rewrites within 10 percent of the John-start word count, move new continuity obligations only forward, and never stage, commit, or push.
---
...
- `John brainstorm(chapter N)` or an explicit brainstorming instruction: perform John's native diagnosis and self-arbitration without editing any manuscript file. Write only `notes/omega/chapter-N/john.md`, unless KingJack supplies an explicit round-private report path; that path overrides the default.
- An explicit user constraint overrides this workflow. Record it in status.

## Brainstorm isolation

In brainstorm mode, treat the resolved Omega report as John's private channel. Never list, open, search, quote, diff, or infer from Patrick, Theo, Chris, arbitration, production, status, score, diagnosis, or proposed-wording files in that round. Do not use another agent's conclusion even if it is visible in inherited conversation; quarantine it. Only a neutral arbiter may compare completed reports.

Apart from John's own report, brainstorm mode is read-only. Do not create or update `notes/john/chapter-N-status.md`, edit a chapter or ledger, or stage anything. Use the target chapter, the user's explicit instructions and locks, and only indispensable canon as evidence.
...
For a full John pass, use `notes/john/chapter-N-status.md` as the durable checkpoint. Create it before substantive edits and update it after every phase and material ruling. In brainstorm mode, use only the resolved private Omega report and never create a John status file.
...
### Brainstorm-mode projection

When invoked in brainstorm mode, project John's native method into a read-only trace:

1. establish the current-text baseline and unchanged word count;
2. diagnose the reading experience and map causal, emotional, scene, room, proof, setup/payoff, and continuity architecture;
3. score the current text honestly with the Phase 6B rubric, without running a rewrite/rescore loop;
4. generate perpendicular candidates only where the diagnosis proves invention is needed;
5. self-arbitrate a compatible, bounded intervention package;
6. record strengths, frozen elements, forward obligations, rejected alternatives, risks, and validation.

Do not draft replacement prose except for a short example indispensable to defining an intervention. End with `State: COMPLETE`, unchanged manuscript movement, and explicit firewall compliance. Then stop; arbitration and production belong to KingJack.
